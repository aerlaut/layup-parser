...
6. If the user would like to address the issue in separate sessions or asks to save the plan, save each task into each separate Markdown files under `.pi/todo/`. Make sure to include a brief section on the motiviation behind the task to provide context. Create a git commit with a short descriptive message to save the plans and ensure that they're not deleted by mistake.