...
:class:`~layup_python.parser.base.LanguageParser` for a specific language,
then call its :meth:`~layup_python.parser.base.LanguageParser.parse` method
to obtain a fully-populated :class:`~layup_python.models.ParsedPackage`.

Supported languages
-------------------
- ``"python"``  — via :class:`~layup_python.parser.python.PythonParser`

Adding a new language
---------------------
1. Create ``layup_python/parser/<lang>/`` with a class implementing the
   :class:`~layup_python.parser.base.LanguageParser` protocol.
...
def get_parser(lang: str) -> LanguageParser:
    """Return a :class:`~layup_python.parser.base.LanguageParser` instance for *lang*.
